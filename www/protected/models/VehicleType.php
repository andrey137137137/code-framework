...
class VehicleType extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title', 'unique',
					'message' => Crud::getMessage('Тип танка', 'unique'),
			),
			array('title, priority_id', 'required'),
			array('title', 'length', 'max' => 77),
			array('priority_id', 'numerical', 'integerOnly' => true),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, priority_id', 'safe', 'on' => 'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'vehicles' => array(self::HAS_MANY, 'Vehicle', 'type_id'),
